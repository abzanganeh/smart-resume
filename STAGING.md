...
- [ ] Free-tier registration grant is **6 credits**: `POST /api/auth/register` response contains `credit_balance: 6`. `GET /api/billing/free-tier` returns `starting_credits: 6`. If staging still returns 3, PATCH `/api/admin/credits/free-grant` — existing `tier_limits_config` rows are not overwritten by a code-seed change.
