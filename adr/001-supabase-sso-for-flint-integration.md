# ADR-001: Supabase SSO for Smart Resume + Flint
...
Flint and Smart Resume serve the same job-seeker journey but today use **different auth systems**:

| Product | Auth today | Identity store |
|---------|------------|----------------|
| **Flint** (desktop) | Supabase GoTrue | `auth.users` via Supabase |
| **Smart Resume** (web) | Custom HS256 JWT (`AUTH_SECRET`, python-jose) | Postgres `users` + `refresh_tokens` |
...
- Post-interview digest sync must attach to the correct Smart Resume account
- No permanent dual-auth bridge (Redis handoff tokens are for Phase 1 import only, not identity)

Three options were considered:

1. **Token exchange** — Smart Resume issues short-lived cross-product tokens; Flint redeems for a session. Lower migration cost; permanent integration surface.
2. **Supabase SSO for both** — Smart Resume migrates to GoTrue; Flint unchanged. Single identity; higher upfront migration cost.
3. **Dual independent auth** — Phase 1 Redis bridge only. Blocks shared billing and creates long-term ops debt.

## Decision

**Migrate Smart Resume to Supabase GoTrue.** Flint keeps its existing Supabase auth implementation.
...
- Smart Resume must migrate off HS256 JWT + `refresh_tokens` table (2–3 weeks in Phase 3)
...
- Local dev requires Supabase CLI (or shared dev project) for Smart Resume, not only `AUTH_SECRET`
...
- Phase 3 adds `EntitlementChecker` calling Smart Resume `GET /api/user/entitlements` with the Supabase JWT.

### Chrome extension (Phase 2 → 3 transition)

- **Phase 2 MVP:** may use existing Smart Resume `/api/auth/login` JWT until Phase 3 lands.
...
- RLS on all Smart Resume Postgres tables unchanged; `auth.uid()` becomes canonical user reference
...
- [ ] New Smart Resume signup creates `auth.users` row
