    ...
    <div>
      ...
      {open && (
        <div className="mt-2 bg-slate-900 border border-slate-800 rounded-lg overflow-hidden">
          {versions.length === 0 ? (
            <p className="text-slate-500 text-xs p-3">No versions saved yet.</p>
          ) : (
            versions.map((v) => (
              <div key={v.snapshot_id} className="flex items-center justify-between p-3 border-b border-slate-800 last:border-0">
                <div>
                  <p className="text-slate-300 text-xs font-medium">v{v.version} — {v.label}</p>
                  <p className="text-slate-600 text-xs">{new Date(v.created_at).toLocaleString()}</p>
                </div>
