...
export function MetricsGate({
  ...
  return (
    <section className="rounded-xl border border-amber-400/30 bg-amber-500/5 dark:bg-amber-400/5 p-4 space-y-3">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-slate-600 dark:text-slate-400 mt-0.5">
            Phase 3 found numbers that appear inherited from a previous AI rewrite. Replace each with a metric you can back up — or leave blank to move it to <span className="text-amber-700 dark:text-amber-300">Metrics Needed</span>. Phase 3 will not carry forward unconfirmed numbers.
          </p>
        </div>
      </div>
            ...
            >
              ...
              >
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <span className="font-medium text-slate-800 dark:text-slate-200 truncate">{entry.scope}</span>
              ...
              {isOpen && (
                <div className="px-3 pb-3 space-y-3 border-t border-slate-300 dark:border-slate-700/50 pt-3">
                  {/* Flagged bullets */}
                  <div className="space-y-1.5">
                    {entry.flags.map((f, i) => (
                      <div key={i} className="flex items-start gap-2">
                        <span className="mt-0.5 shrink-0 text-[10px] font-medium px-1.5 py-0.5 rounded bg-amber-500/15 dark:bg-amber-400/15 text-amber-700 dark:text-amber-300 uppercase tracking-wide">
                          {REASON_LABELS[f.reason]}
                        </span>
                        <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">{f.bullet}</p>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="space-y-2">
                    <div>
                      <label className="block text-xs text-slate-600 dark:text-slate-400 mb-1">
                        Verified metric <span className="text-slate-600 dark:text-slate-400">(e.g. "Test F1 0.64 on 1.8M transactions")</span>
                      </label>
                      <input
                        type="text"
                        value={entry.metric}
                        ...
                        className="w-full rounded-md bg-slate-100 dark:bg-slate-800 border border-slate-400 dark:border-slate-600 px-2.5 py-1.5 text-sm text-slate-800 dark:text-slate-200 placeholder-slate-600 focus:outline-none focus:border-slate-400"
                      />
                    </div>
                    <div>
                      <label className="block text-xs text-slate-600 dark:text-slate-400 mb-1">
                        Source note <span className="text-slate-600 dark:text-slate-400">(optional — not sent to AI, for your reference)</span>
                      </label>
                      <input
                        type="text"
                        ...
                        className="w-full rounded-md bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 px-2.5 py-1.5 text-sm text-slate-600 dark:text-slate-400 placeholder-slate-700 focus:outline-none focus:border-slate-500"
                      />
                    </div>
                  </div>
                </div>
      ...
      <div className="flex items-center justify-between pt-1">
        ...
        <button
          ...
          className={cn(
            ...
            savedOk
              ? "bg-emerald-600/20 text-emerald-700 dark:text-emerald-300 border border-emerald-600/30"
              : "bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-800 dark:text-slate-200 border border-slate-400 dark:border-slate-600"
          )}
        >
          {saving ? (
