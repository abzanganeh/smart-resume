...
export function VoiceTab({
  ...
  if (voiceState === "idle") {
    return (
      <div className="flex flex-col items-center gap-4 py-10 border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-xl">
        <Mic className="w-10 h-10 text-slate-600 dark:text-slate-400" />
        <p className="text-slate-700 dark:text-slate-300 font-medium">Record your resume by voice</p>
        <p className="text-slate-600 dark:text-slate-400 text-sm text-center max-w-xs">
          {supportsWebSpeech
            ? "Speak naturally — your words appear live, free on every plan."
            : "Record audio, then we'll transcribe it using Whisper."}
        </p>
        {!supportsWebSpeech && (
          <p className="text-slate-600 dark:text-slate-400 text-xs text-center">
            Live transcription requires Chrome or Edge. Whisper transcription is included with
            paid plans.
          </p>
        )}
        <button
        ...
        {error && (
          <p className="text-red-700 dark:text-red-400 text-sm text-center px-4">{error}</p>
        )}
      </div>
    );
  }
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-3 px-4 py-2 rounded-lg bg-red-500/10 border border-red-500/30">
          ...
          <button
            ...
            className="ml-auto px-4 py-1.5 bg-slate-200 dark:bg-slate-700 hover:bg-slate-600 text-slate-900 dark:text-white text-xs font-semibold rounded-full transition-colors"
          >
            Stop
          </button>
        </div>
        ...
        {/* Live transcript box */}
        <div className="min-h-32 bg-slate-100/60 dark:bg-slate-800/60 border border-slate-300 dark:border-slate-700 rounded-xl p-4 text-sm leading-relaxed">
          {finalText || interimText ? (
            ...
            <span className="text-slate-600 dark:text-slate-400 italic">Start speaking — your words appear here…</span>
          )}
        </div>
      ...
      <div className="flex flex-col items-center gap-4 py-10 border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-xl">
        <div className="relative flex items-center justify-center">
          ...
          <div className="relative w-16 h-16 rounded-full bg-red-500 flex items-center justify-center">
            <MicOff className="w-7 h-7 text-slate-900 dark:text-white" />
          </div>
        </div>
        <p className="text-red-700 dark:text-red-400 font-semibold tabular-nums text-lg">{durationLabel}</p>
        <p className="text-slate-600 dark:text-slate-400 text-sm">Recording… speak your career history</p>
        <button
          type="button"
          onClick={() => void handleStop()}
          className="px-6 py-3 bg-slate-200 dark:bg-slate-700 hover:bg-slate-600 text-slate-900 dark:text-white font-semibold rounded-full transition-colors flex items-center gap-2"
        >
          Stop &amp; transcribe
        </button>
      </div>
  ...
  if (voiceState === "transcribing") {
    return (
      <div className="flex flex-col items-center gap-3 py-12 border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-xl">
        <div className="w-8 h-8 border-2 border-amber-400 border-t-transparent rounded-full animate-spin" />
        <p className="text-slate-700 dark:text-slate-300 text-sm">Transcribing with Whisper…</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      <p className="text-slate-600 dark:text-slate-400 text-sm">Review and edit, then continue.</p>
      <textarea
        value={finalText}
      ...
      <div className="flex items-center justify-between gap-3">
        <button
          ...
          onClick={reset}
          disabled={disabled}
          className="px-4 py-2 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 text-sm font-medium disabled:opacity-40 flex items-center gap-1.5"
        >
